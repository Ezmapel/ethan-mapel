...
const pairs = [];
data.forEach(obj2 => {
  pairs.push(obj2.address.geo.lat, obj2.address.geo.lng);
});
console.log(pairs);

// object destructuring is as follows
const pairs2 = [];
data.forEach(({ address }) => pairs2.push(address.geo));
console.log(pairs2);

// create an array of strings that combine the company's catchPhrase and bs
const strings = [];
data.forEach(({ company }) =>
  strings.push(company.catchPhrase + " " + company.bs)
);
console.log(strings);

// query the HTML for the class class-week, then use forEach to change the text content
let weeks = document.querySelectorAll(".class-week");
weeks.forEach(week => (week.textContent = "Something else"));
